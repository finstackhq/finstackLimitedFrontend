...
export function KYCPopup() {
  ...
  const checkAndShowPopup = () => {
    try {
      ...
      if (isVerified === null || status === null) return;

      // If kycStatus is approved (or any pending state), NEVER show popup
      // This takes priority over isKycVerified, which can be unreliable
      const isPending =
        status === "pending" ||
        status === "submitted" ||
        status === "in_review" ||
      ...
      // Only show popup if explicitly unverified
      if (isVerified === "false") {
        setIsVisible(true);
      }
    } catch (e) {
  ...
  useEffect(() => {
    // Case 1: Already logged in + page refresh — localStorage has real data
    checkAndShowPopup();

    // Case 2: Fresh login — auth-form fires this after writing real KYC data
    window.addEventListener("kyc-ready", checkAndShowPopup);
    return () => window.removeEventListener("kyc-ready", checkAndShowPopup);
  }, []);
          ...
          >
            ...
            <div className="flex flex-col items-center text-center p-8 pt-10">
              ...
              <p className="text-muted-foreground mb-8">
                Your account is currently unverified. Please complete the KYC
                process to unlock all features and ensure account security.
              </p>
              ...
              <div className="flex flex-col w-full gap-3">
                ...
                >
                  <Link href="/dashboard/settings">Verify Now</Link>
                </Button>
                <Button
                  variant="ghost"
                  onClick={() => setIsVisible(false)}
