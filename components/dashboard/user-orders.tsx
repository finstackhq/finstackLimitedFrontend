...
export function UserOrders() {
    ...
    const getStatusColor = (status: string) => {
        ...
        if (s.includes("PENDING") || s.includes("PROCESSING")) return "bg-yellow-500/10 text-yellow-500 border-yellow-500/20"
            ...
            <Card className="border-0 shadow-sm bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm">
                ...
                <CardContent>
                    <div className="space-y-3">
                        {[1, 2, 3].map((i) => (
                            <div key={i} className="h-16 w-full bg-gray-100 dark:bg-gray-800/50 animate-pulse rounded-lg" />
                        ))}
                    </div>
                </CardContent>
            </Card>
        ...
        return (
            <Card className="border-red-100 dark:border-red-900/50 bg-red-50/50 dark:bg-red-900/10">
                <CardContent className="pt-6 flex flex-col items-center justify-center text-center gap-2">
                    ...
                    <Button variant="outline" size="sm" onClick={fetchOrders} className="mt-2 text-red-600 border-red-200 hover:bg-red-100 dark:border-red-800 dark:hover:bg-red-900/50">
                        Try Again
                    </Button>
                </CardContent>
            </Card>
                            ...
                            <div className="flex items-start sm:items-center gap-3 sm:gap-4 w-full sm:w-auto">
                                ...
                                <div className="space-y-1 min-w-0 flex-1">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        <Badge variant="outline" className="text-[10px] h-5 px-1.5 py-0 border-gray-200 dark:border-gray-700 text-muted-foreground rounded-md font-mono truncate max-w-[120px] sm:max-w-none">
                                            {order.reference}
                                        </Badge>
                                    </div>
