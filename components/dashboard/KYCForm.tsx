...
export function KYCForm() {
  ...
  const [livelinessProviderReference, setLivelinessProviderReference] =
    useState<string>("");
  const [livelinessConfidence, setLivelinessConfidence] = useState<
    number | null
  >(null);
    ...
    switch (step) {
      case 1:
        ...
        if (!kycSessionId || !sessionExpiresAt) {
          toast({
            title: "Start Liveliness",
            description: "Create a KYC session first",
            variant: "destructive",
          });
          return false;
        }
      ...
      case 2:
        ...
        if (!formData.selfie) {
          toast({
            ...
            description: "Select a selfie image to proceed",
            variant: "destructive",
          });
          return false;
        }
      ...
      case 3:
        // Step 3: Liveliness check disabled (bypassed)
        /*
        if (!livelinessProviderReference || (livelinessConfidence ?? 0) < 60) {
          toast({
            title: "Liveliness Not Verified",
            description: "Please complete liveness check (≥60%)",
            variant: "destructive",
          });
          return false;
        }
      ...
      case 5:
        ...
        if (
          ...
          toast({
            title: "Incomplete Information",
            description:
              "Please fill in all required location information fields",
            variant: "destructive",
          });
          return false;
        }
      ...
      case 6:
        ...
        const isNigerian =
          ...
          (formData.countryOfResidence || "").toLowerCase() === "nigeria";
        if (
          !formData.idType ||
          ...
          toast({
            title: "Incomplete Information",
            description: "Fill ID type, issuing country, and ID number",
            variant: "destructive",
          });
          return false;
        }
        if (isNigerian) {
          if (!formData.bvn || !formData.ninNumber) {
            toast({
              title: "BVN and NIN required",
              description: "Provide BVN and NIN for Nigerian verification",
              variant: "destructive",
            });
            return false;
          }
          ...
          if (!formData.idExpiry) {
            toast({
              title: "ID Expiry required",
              description:
                "Provide ID expiry date for non-Nigerian verification",
              variant: "destructive",
            });
            return false;
          }
          if (!formData.idFront || !formData.idBack || !formData.selfie) {
            toast({
              ...
              description:
                "Upload ID front, ID back, and selfie for non-Nigerian verification",
              variant: "destructive",
            });
            return false;
          }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isNigerian) {
        ...
        form.append(
          "country",
          formData.countryOfResidence || formData.nationality,
        );
          ...
          try {
            ...
            if (raw) {
              ...
              const candidateRef =
                ...
                parsed?.liveliness_provider_reference ??
                parsed?.providerReference;
              if (!refToSend && candidateRef) refToSend = String(candidateRef);
              const candidateConf =
                parsed?.confidence ?? parsed?.liveliness_confidence;
              const numConf =
                typeof candidateConf === "number"
                  ? candidateConf
                  : Number(candidateConf);
              if (typeof confToSend !== "number" && !Number.isNaN(numConf))
                confToSend = numConf;
            }
          } catch {}
        }
        if (refToSend)
          form.append("liveliness_provider_reference", String(refToSend));
        // if (typeof confToSend === "number")
        //   form.append("liveliness_confidence", String(confToSend));
        ...
        form.append(
          "country",
          String(formData.countryOfResidence || formData.nationality || ""),
        );
        form.append("id_type", String(idTypeNormalized || ""));
        form.append("id_number", String(formData.idNumber || ""));
        if (formData.idExpiry)
          form.append("id_expiry", String(formData.idExpiry));
        ...
        if (formData.selfie)
          form.append(
            "selfie",
            formData.selfie,
            (formData.selfie as any).name || "selfie.jpg",
          );
        ...
        if (formData.idFront)
          form.append(
            "proof_id_front",
            formData.idFront,
            (formData.idFront as any).name || "id_front.jpg",
          );
        if (formData.idBack)
          form.append(
            "proof_id_back",
            formData.idBack,
            (formData.idBack as any).name || "id_back.jpg",
          );
    ...
    const tick = () => {
      ...
      if (remaining === 0) {
        ...
        toast({
          ...
          description: "Start a new liveness session",
          variant: "destructive",
        });
      }
    };
  ...
  const startLivelinessSession = async () => {
    try {
      ...
      const res = await fetchWithAuth("/api/fstack/kycSession", {
        method: "POST",
        cache: "no-store",
        credentials: "include",
      });
      ...
      if (!res.ok)
        throw new Error((data as any)?.message || "Failed to start session");
      const sessionId =
        (data as any)?.kyc_session_id || (data as any)?.sessionId;
      const expiresAt = (data as any)?.expiresAt
        ? new Date((data as any)?.expiresAt).getTime()
        : Date.now() + 5 * 60 * 1000;
      if (!sessionId) throw new Error("kyc_session_id missing");
      setKycSessionId(sessionId);
      setSessionExpiresAt(expiresAt);
      toast({
        title: "Liveliness Session Started",
        description: "Session active for 5 minutes",
      });
      setCurrentStep(2);
    } catch (e: any) {
      toast({
  ...
  const uploadSelfieToCloudinary = async (): Promise<string> => {
    ...
    try {
      ...
      const res = await fetchWithAuth("/api/cloudinary/upload", {
        method: "POST",
        body: fd,
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error((data as any)?.error || "Cloudinary upload failed");
  ...
  const runLivelinessCheck = async () => {
    try {
      ...
      const ref =
        (data as any)?.liveliness_provider_reference ??
        (data as any)?.provider_reference ??
        (data as any)?.providerReference;
      const conf = Number(
        (data as any)?.liveliness_confidence ?? (data as any)?.confidence ?? 0,
      );
      ...
      //     description: "Please retake your selfie (confidence < 60%)",
      //     variant: "destructive",
      //   });
      // } else {
      //   toast({
      //     title: "Liveliness Verified",
      //     description: `Confidence ${conf}%`,
      //   });
      //   setCurrentStep(4);
      // }
    } catch (e: any) {
      toast({
        title: "Liveliness Error",
        description: e?.message || "Please try again",
        variant: "destructive",
      });
    } finally {
        ...
        <div className="text-center py-12">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 max-w-md mx-auto">
            ...
            <p className="text-xs text-yellow-700 mt-2">
              This usually takes 24-48 hours. You'll be notified once the review
              is complete.
            </p>
          </div>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-red-800 mb-2">
                  Unfortunately, your KYC application was not approved. Please
                  review the reason below and resubmit with the correct
                  information.
                </p>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2 mb-6 pb-3 border-b border-gray-200">
                ...
                <h3 className="text-xl font-bold text-gray-900">
                  Liveness Check: Start Session
                </h3>
              </div>
              <div className="space-y-3">
                <p className="text-sm text-gray-600">
                  Start a KYC liveliness session. This creates a temporary
                  session and enables selfie verification.
                </p>
                <div className="flex items-center gap-4">
                  <Button
                    onClick={startLivelinessSession}
                ...
                {sessionExpiresAt && (
                  <p className="text-xs text-gray-500">
                    Session expires in {Math.floor(countdown / 60)}:
            ...
            <div className="space-y-6">
              <div className="mb-6 pb-3 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-600 pl-8">
                  Choose a clear selfie image. You will upload it in the next
                  step.
                </p>
                {sessionExpiresAt && (
          ...
          {currentStep === 3 && (
            <div className="space-y-6">
              <div className="flex items-center gap-2 mb-6 pb-3 border-b border-gray-200">
                ...
                <h3 className="text-xl font-bold text-gray-900">
                  Upload & Run Liveliness
                </h3>
              </div>
              <div className="space-y-4">
                <p className="text-sm text-gray-600">
                  we would check your seldie and verify liveness
                </p>
                <div className="flex items-center gap-4">
                  <Button
                    onClick={runLivelinessCheck}
                    disabled={
                      checkingLiveliness || !formData.selfie || !kycSessionId
                    }
                ...
                {typeof livelinessConfidence === "number" && (
                  <div
                    className={`text-sm ${livelinessConfidence < 60 ? "text-red-600" : "text-green-600"}`}
                  >
                    Confidence: {livelinessConfidence}%{" "}
                    {livelinessProviderReference &&
                      `• Ref: ${livelinessProviderReference.slice(0, 12)}…`}
                  </div>
                )}
                */}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(val) =>
                      updateField("countryOfResidence", val)
                    }
                  >
                    <SelectTrigger className="border-gray-200">
                ...
                <div>
                  ...
                  {formData.countryOfResidence === "Nigeria" ? (
                    <Select
                      ...
                      onValueChange={(val) => updateField("stateRegion", val)}
                    >
                      <SelectTrigger className="border-gray-200">
                    ...
                    <Input
                      ...
                      value={formData.stateRegion}
                      onChange={(e) =>
                        updateField("stateRegion", e.target.value)
                      }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(val) => updateField("idType", val)}
                  >
                    <SelectTrigger className="border-gray-200">
                  ...
                  (formData.countryOfResidence || "").toLowerCase() !==
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateField("idExpiry", e.target.value)
                        }
                        className="border-gray-200"
                      />
                    </div>
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateField("ninNumber", e.target.value)
                        }
