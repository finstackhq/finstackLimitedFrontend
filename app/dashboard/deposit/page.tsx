...
// TESTING THE 13/07 paycrest dynamic account
"use client";
...
interface DepositAddressResponse {
  ...
  currency: string;
  provider: string;
  // NGN virtual account fields (Blockradar — currently unused, kept for the interface shape)
  accountNumber?: string;
  accountName?: string;
  bankName?: string;
  message?: string;
...
interface OnrampOrderResponse {
  ...
  currency?: string;
  stablecoin?: string;
  // Paycrest V2 providerAccount fields (flat, matches initiatePaycrestOnramp's response)
  bankName?: string;
  accountNumber?: string;
  accountName?: string;
  amountToTransfer?: string;
...
export default function DepositPage() {
  ...
  // ── Check for a primary bank account on mount — Paycrest needs one as refundAccount ──
  useEffect(() => {
    const checkPrimaryBank = async () => {
      try {
        const res = await fetch("/api/fstack/deposit?type=wallet");
        const data = await res.json();
        setHasPrimaryBank(!!data.hasPrimaryBank);
      } catch {
        // Non-blocking — continue anyway, backend will still validate on submit
      }
    };
    checkPrimaryBank();
  }, []);
  ...
  const handleMethodSelect = (method: DepositMethod) => {
    setDepositMethod(method);
    setError(null);
    // Both methods now just advance to their first data-entry step.
    // NGN no longer eagerly fetches anything here — Paycrest onramp needs an
    // amount + stablecoin from the user before it can create an order.
    setCurrentStep(1);
  };

  // Fetch deposit address — used only by the crypto (USDC/CNGN) flow now.
  const fetchDepositAddress = async (
    currency: string,
  ): Promise<DepositAddressResponse> => {
    const res = await fetch(`/api/fstack/deposit?currency=${currency}`);
  ...
  // NGN: create a one-time Paycrest onramp order — matches initiatePaycrestOnramp
  // in walletController.js, which expects { amount, fiatCurrency, stablecoin }.
  const handleNgnInitiate = async () => {
    if (!ngnStablecoin || !validateAmount(ngnAmount)) return;
    setLoading(true);
    setError(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="max-w-2xl mx-auto p-6 shadow-lg border-gray-200 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {/* ══ STEP 2 (NGN): Pay into one-time Paycrest account ═══════════════ */}
        {currentStep === 2 && depositMethod === "naira" && onrampOrder && (
          <div className="space-y-6">
            <div className="text-center">
              <div className="w-14 h-14 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-3">
            ...
            {onrampOrder.accountNumber ? (
              <div className="rounded-2xl border border-gray-200 overflow-hidden">
                <div className="bg-gradient-to-r from-green-600 to-green-500 px-5 py-4 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-white font-semibold text-sm">
                      {onrampOrder.bankName || "Virtual Account"}
                    </p>
                    <p className="text-white/70 text-xs">
                      One-time account · This order only
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-5 space-y-4">
                  ...
                  {onrampOrder.validUntil && (
                    <div className="flex items-center gap-2 text-xs text-amber-600">
                      ...
                      <span>
                        This account expires at{" "}
                        {new Date(onrampOrder.validUntil).toLocaleTimeString(
                          [],
            ...
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl space-y-2">
              ...
              <ul className="text-xs text-blue-700 space-y-1.5 list-disc list-inside">
                ...
                <li>
                  Your {onrampOrder.stablecoin || ngnStablecoin} wallet will be
                  credited automatically
                </li>
                <li>
                  Use this account for this transaction only — it's unique to
                  this order
                </li>
