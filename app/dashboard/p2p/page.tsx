...
import { P2PAd, P2POrder, PaymentMethod, CountryCode, Merchant } from "@/lib/p2p-types"
...
export default function P2PMarketplacePage() {
  ...
  const fetchAds = async (page = 1, isLoadMore = false) => {
    try {
      ...
      console.log("P2P API Response:", json); // Debug logging
      ...
      if (json.success && Array.isArray(json.data)) {
        ...
        console.log("Mapped P2P Ads:", newAds);
        ...
        if (isLoadMore) {
           ...
           console.warn("P2P API returned unexpected format:", json);
           if (!isLoadMore) setError("Failed to load orders. Please try again later.")
      }
    } catch (error) {
          ...
          <div className="space-y-4">
            ...
            {pagination.totalPages > 1 && pagination.page < pagination.totalPages && (
                <div className="flex justify-center pt-6 border-t border-gray-100">
                    ...
                    >
                        {loadingMore ? 'Loading...' : `Load More (${pagination.total - (pagination.page * pagination.limit)} remaining)`}
                    </Button>
                </div>
            )}
          </div>
