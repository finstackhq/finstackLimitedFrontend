function getCurrencySymbol(currency: string): string {
  switch (currency) {
    ...
    case "RMB":
      return "¥";
    default:
      return currency + " "; // Returns "KES " if unknown
  }
}
("use client");
...
export default function P2PMarketplacePage() {
  ...
  const filterAds = (ads: P2PAd[], type: "buy" | "sell") => {
    return ads
      ...
      .filter(
        (ad) =>
          filterPair === "all" ||
          `${ad.cryptoCurrency}/${ad.fiatCurrency}` === filterPair,
      )
      .filter(
        (ad) =>
          filterPayment === "all" ||
      ...
      .filter((ad) => {
        if (
          ...
          ad.price < parseFloat(minPrice)
        )
          return false;
        if (
          maxPrice &&
          parseFloat(maxPrice) > 0 &&
          ad.price > parseFloat(maxPrice)
        )
          return false;
        return true;
      })
  ...
  const uniquePairs = Array.from(
    new Set(mockP2PAds.map((ad) => `${ad.cryptoCurrency}/${ad.fiatCurrency}`)),
  );
  const uniqueCountries = Array.from(
    new Set(mockP2PAds.map((ad) => ad.country)),
  );
  ...
  const renderAdRow = (ad: P2PAd, actionLabel: string, actionColor: string) => {
    ...
    // Special display for CNGN/USD and USD/CNGN pairs
    let priceDisplay;
    if (
      (ad.cryptoCurrency === "CNGN" && ad.fiatCurrency === "USD") ||
      (ad.cryptoCurrency === "USD" && ad.fiatCurrency === "CNGN")
    ) {
      // If price is CNGN per USD, show: 1 USD = X CNGN
      // If price is USD per CNGN, show: 1 CNGN = X USD
      if (ad.cryptoCurrency === "CNGN" && ad.fiatCurrency === "USD") {
        priceDisplay = `1 USD = ${ad.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 3 })} CNGN`;
      } else if (ad.cryptoCurrency === "USD" && ad.fiatCurrency === "CNGN") {
        // Show reciprocal
    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-600">
            {merchant.completionRate}% completion
          </p>
        </div>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-600">
                  {buyAds.length} {buyAds.length === 1 ? "offer" : "offers"}{" "}
                  available
                </span>
              </div>
          ...
          <Card className="shadow-lg border-gray-200">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-600">
                  {sellAds.length} {sellAds.length === 1 ? "offer" : "offers"}{" "}
                  available
                </span>
              </div>
