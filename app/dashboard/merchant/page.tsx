...
export default function MerchantDashboard() {
      ...
      try {
        ...
        if (data.success && data.data) {
          ...
          if (userRole === "merchant" && isKyc) {
            ...
            try {
              localStorage.setItem("merchant-status", "pending_kyc");
            } catch {}
          } else {
  ...
  useEffect(() => {
    const fetchWalletBalances = async () => {
      try {
        ...
        const balancesList: any[] = Array.isArray(json?.data)
          ? json.data
        ...
        if (balancesList.length > 0) {
          ...
          balancesList.forEach((bal: any) => {
            ...
            const balanceAmount =
              typeof bal.balance === "object"
                ? Number(bal.balance?.total) ||
                  Number(bal.balance?.available) ||
                  0
                : Number(bal.balance) || 0;

            if (currency === "NGN") balances.NGN = balanceAmount;
            if (currency === "USDT" || currency === "USDC")
              balances.USDT = balanceAmount;
    ...
    const fetchOrderStats = async () => {
      try {
        ...
        if (json.success && Array.isArray(json.trades)) {
          ...
          const activeOrders = json.trades.filter(
            (trade: any) =>
              trade.status === "PENDING_PAYMENT" ||
              trade.status === "AWAITING_RELEASE",
          ).length;
          const completedToday = json.trades.filter(
            (trade: any) =>
    ...
    const fetchBankAccounts = async () => {
      ...
      try {
        ...
        if (data.success && Array.isArray(data.data)) {
          const mappedAccounts: BankAccount[] = data.data.map(
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("merchant-ad-published", handler as any);
  }, []);
  ...
  const completionRate = (
    (merchantStats.completedTrades / merchantStats.totalTrades) *
    100
  ).toFixed(1);
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 space-y-6">
        <div className="text-center md:text-left">
          ...
          <p className="text-sm md:text-base text-gray-600">
            Become a merchant to unlock P2P trading features
          </p>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Status: {merchantStatus.replace("_", " ")}
              </p>
              <p className="text-sm text-gray-600">
    ...
    <div className="max-w-7xl mx-auto px-4 md:px-6 space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-lg md:text-xl font-semibold">
                {loadingTradeSummary ? "..." : tradeSummary.totalAds}
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-3 md:p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-lg md:text-xl font-semibold">
                {loadingTradeSummary ? "..." : tradeSummary.totalTrades}
              </p>
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-lg md:text-xl font-semibold text-green-600">
                {loadingTradeSummary ? "..." : tradeSummary.totalCompleted}
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-3 md:p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-lg md:text-xl font-semibold">
                {loadingTradeSummary ? "..." : tradeSummary.activeOffers}
              </p>
            </div>
          </div>
        </Card>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="p-4 md:p-6">
          ...
          <Link href="/dashboard/merchant/orders">
            <Button className="w-full mt-4">Manage Orders</Button>
          </Link>
        </Card>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-right">
                ...
                <button
                  onClick={() =>
                    handleCopy(walletBalance.NGN.toString(), "ngn")
                  }
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-right">
                <p className="text-xl font-bold">
                  {showBalance
                    ? `$${walletBalance.USDT.toLocaleString()}`
                    : "••••••"}
                </p>
                <button
                  onClick={() =>
                    handleCopy(walletBalance.USDT.toString(), "usdt")
                  }
                  className="text-xs text-gray-500 hover:text-blue-600 flex items-center gap-1"
                >
                  {copied === "usdt" ? (
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              ...
              <div className="text-right">
                ...
                <button
                  onClick={() =>
                    handleCopy(walletBalance.CNGN.toString(), "cngn")
                  }
                  className="text-xs text-gray-500 hover:text-blue-600 flex items-center gap-1"
                >
                  {copied === "cngn" ? (
      ...
      <Card className="p-4 md:p-6">
        ...
        {ads.length === 0 ? (
          ...
          <div className="space-y-2">
            {ads.map((ad) => {
              // Show rate as '1 [fiat] = X CNGN' for fiat/CNGN pairs
              let rateDisplay = null;
              const [crypto, fiat] = ad.pair.split("/");
              const cngnFiatList = ["USD", "RMB", "GHS", "XAF", "XOF"];
              if (crypto === "CNGN" && cngnFiatList.includes(fiat)) {
                rateDisplay = `1 ${fiat} = ${ad.fixedPrice?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} CNGN`;
              }
              return (
                <div
                  key={ad.id}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-600">
                        Limits: {ad.minLimit ?? "—"} - {ad.maxLimit ?? "—"} |
                        Methods: {ad.paymentMethods.join(", ")}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                ad from the marketplace and stop any new trades from being
                created.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <BecomeMerchantModal
        ...
        onSubmitted={() => {
          ...
          try {
            localStorage.setItem("merchant-status", "pending");
          } catch {}
        }}
      />
...
function MerchantAdWizardWrapper({
  onPublish,
}: {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-gray-500 mt-3">
        After you click "Publish Ad" above, it will be added to your My Ads
        list.
      </p>
