...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!token) {
      return NextResponse.json(
        { success: false, error: "Unauthorized: No access token found" },
        { status: 401 },
      );
    }
    ...
    if (!response.ok) {
      ...
      console.error(
        "Failed to create ad in backend:",
        response.status,
        errorText,
      );
      try {
        ...
        return NextResponse.json(
          {
            success: false,
            error: `Backend returned ${response.status}: ${JSON.stringify(errorJson)}`,
          },
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        "Failed to update ad in backend:",
        response.status,
        errorText,
      );
      try {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        "Failed to delete ad in backend:",
        response.status,
        errorText,
      );
      try {
