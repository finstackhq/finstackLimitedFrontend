  ...
  try {
    ...
    if (isJson) {
      ...
      if (country !== "nigeria") {
        ...
        const livelinessRef =
          body?.liveliness_reference ??
          body?.liveliness_provider_reference ??
          body?.provider_reference ??
          body?.providerReference;
        const livelinessConfidence =
          body?.liveliness_confidence ?? body?.confidence;
        ...
        if (
          typeof livelinessConfidence !== "undefined" &&
          livelinessConfidence !== null
        ) {
      ...
      const livelinessConfidence =
        body?.liveliness_confidence ?? body?.confidence;
      if (typeof livelinessRef !== "undefined" && livelinessRef !== null)
        formData.append("liveliness_provider_reference", String(livelinessRef));
      if (
        typeof livelinessConfidence !== "undefined" &&
        livelinessConfidence !== null
      )
        formData.append("liveliness_confidence", String(livelinessConfidence));
    ...
    for (const k of forwardKeys) {
      ...
      if (val !== null)
        upstreamForm.append(k, typeof val === "string" ? val : String(val));
    }
    ...
    if (idExpiry !== null)
      upstreamForm.append(
        "id_expiry",
        typeof idExpiry === "string" ? idExpiry : String(idExpiry),
      );
    ...
    if (idBackCandidate && typeof idBackCandidate === "object") {
      ...
      upstreamForm.append(
        "proof_id_back",
        file,
        (file as any).name || "id_back.jpg",
      );
    }

    const livelinessRef =
      incomingFormData.get("liveliness_reference") ??
      incomingFormData.get("liveliness_provider_reference") ??
      incomingFormData.get("provider_reference") ??
      incomingFormData.get("providerReference");
    const livelinessConfidence =
      incomingFormData.get("liveliness_confidence") ??
      incomingFormData.get("confidence");
    if (livelinessRef)
      upstreamForm.append(
        "liveliness_provider_reference",
        String(livelinessRef),
      );
    if (livelinessConfidence)
      upstreamForm.append(
        "liveliness_confidence",
        String(livelinessConfidence),
      );
