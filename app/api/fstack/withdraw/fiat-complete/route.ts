...
export async function POST(request: NextRequest) {
  try {
    if (!baseUrl) {
      console.error(
        "[fstack/withdraw/fiat-complete] FINSTACK_BACKEND_API_URL not set",
      );
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "All fields are required: walletCurrency, fiatCurrency, amount, otpCode, destinationAccountNumber, institutionCode, accountName",
        },
        { status: 400 },
      );
    }
    ...
    console.error(
      "[fstack/withdraw/fiat-complete] POST error:",
      error?.message || error,
    );
    return NextResponse.json(
