...
export async function POST(
  ...
  try {
    ...
    if (!baseUrl) {
      console.error(
        `[fstack/p2p/${tradeId}/cancel] FINSTACK_BACKEND_API_URL not set`,
      );
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      console.error(
        `[fstack/p2p/${tradeId}/cancel] Non-JSON response:`,
    ...
    if (!response.ok) {
      console.error(
        `[fstack/p2p/${tradeId}/cancel] Backend cancellation failed:`,
        data,
      );
      return NextResponse.json(
        {
