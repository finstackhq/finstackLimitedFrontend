...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 1. Fetch Summary (Total Users, Balances)
    const summaryRes = await fetch(`${baseUrl}admin/dashboard-summary`, {
      headers,
      cache: "no-store",
    });
    ...
    // 2. Fetch Volume
    const volumeRes = await fetch(`${baseUrl}admin/volume`, {
      headers,
      cache: "no-store",
    });
    ...
    const transRes = await fetch(`${baseUrl}admin/all-transaction`, {
      headers,
      cache: "no-store",
    });
    const transData = await transRes.json();

    // 4. MAP TRANSACTIONS (Crucial for the RecentTransactions table)
    const rawList = transData.transactions || transData.data || [];
    const mappedTransactions = rawList.slice(0, 5).map((t: any) => ({
      id: t._id || t.id || "—",
      user: t.userId?.email?.split("@")[0] || "User",
    ...
    // FINAL DATA OBJECT - This matches your StatsCards and RecentTransactions props
    const finalData = {
      success: true,
      data: {
        totalUsers: summary.data?.totalUsers || 0,
        suspendedUsers: summary.data?.suspendedUsers || 0,
        pendingKyc: summary.data?.pendingKyc || 0,
        platformBalances: summary.data?.platformBalances || {
          USDC: 0,
          CNGN: 0,
          NGN: 0,
        },
        totalTransactionVolume: volume.data?.totalVolume || 0,
