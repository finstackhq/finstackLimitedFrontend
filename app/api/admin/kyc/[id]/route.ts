...
export async function GET(
  ...
  const id = params?.id;
  const endpoint =
    baseUrl && id ? `${baseUrl}admin/getSingleKyc/${id}` : undefined;

  try {
    if (!endpoint) {
      console.error(
        "[admin/kyc/:id] FINSTACK_BACKEND_API_URL not set or id missing",
      );
      return NextResponse.json(
        { error: "Server not configured or id missing" },
        { status: 500 },
      );
    }
