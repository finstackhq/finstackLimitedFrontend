...
export default function MerchantsPage() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.users) {
            const mappedMerchants: Merchant[] = data.users.map(
              (user: any, index: number) => ({
                ...
                status: (user.status || "pending") as
                  | "verified"
                  | "pending"
                  | "under_review"
                  | "suspended",
                tier: (user.tier || "standard") as
                  | "standard"
                  | "premium"
                  | "enterprise",
                ...
                compliance: {
                  kycStatus: (user.compliance?.kycStatus || "pending") as
                    | "approved"
                    | "pending"
                    | "under_review"
                    | "rejected",
                  amlStatus: (user.compliance?.amlStatus || "pending") as
                    | "approved"
                    | "pending"
                    | "under_review"
                    | "flagged",
                  riskLevel: (user.compliance?.riskLevel || "medium") as
                    | "low"
                    | "medium"
                    | "high",
                },
  ...
  const handleFilter = (filters: any) => {
    ...
    if (filters.status && filters.status !== "all") {
      filtered = filtered.filter(
        (merchant) => merchant.status === filters.status,
      );
    }
    ...
    if (filters.country && filters.country !== "all") {
      filtered = filtered.filter(
        (merchant) => merchant.country === filters.country,
      );
    }

    if (filters.riskLevel && filters.riskLevel !== "all") {
      filtered = filtered.filter(
        (merchant) => merchant.compliance.riskLevel === filters.riskLevel,
      );
    }
    ...
    if (filters.dateRange?.from && filters.dateRange?.to) {
      filtered = filtered.filter((merchant) => {
        ...
        return (
          merchantDate >= filters.dateRange.from &&
          merchantDate <= filters.dateRange.to
        );
      });
    }
        ...
        {merchantStats.map((stat) => (
          ...
          >
            ...
            <div className="flex items-center">
              {stat.trending === "up" ? (
                <svg
                  className="w-3 h-3 md:w-4 md:h-4 text-green-600 mr-1"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <svg
                  className="w-3 h-3 md:w-4 md:h-4 text-red-600 mr-1"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
