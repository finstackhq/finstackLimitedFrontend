...
export default function P2POrdersPage() {
  ...
  const fetchOrders = async (status?: string) => {
    setLoading(true);
    try {
      const url = status ? `/api/admin/p2p?status=${status}` : "/api/admin/p2p";
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        const totalVol = mappedOrders.reduce(
        ...
        const completedToday = mappedOrders.filter(
          (o: any) =>
            o.status === "completed" &&
            new Date(o.date).toDateString() === today,
        ).length;
        const disputed = mappedOrders.filter(
    ...
    <div className="space-y-6">
      <div className="animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        <p className="text-gray-600 mt-1">
          Monitor and manage peer-to-peer trading orders
        </p>
      </div>
      ...
      <div className="animate-in fade-in slide-in-from-top-4 duration-700">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
          {p2pStats.map((stat) => (
            <div
              key={stat.title}
              className="bg-white p-4 md:p-6 rounded-lg border border-gray-200 shadow-sm min-w-0"
            >
              <h3 className="text-xs font-medium text-gray-500 mb-2 truncate">
                {stat.title}
              </h3>
              <p className="text-lg md:text-xl font-semibold text-gray-900 mb-1 truncate">
                {stat.value}
              </p>
              <div className="flex items-center">
                {stat.trend === "up" ? (
                  <svg
                    className="w-3 h-3 md:w-4 md:h-4 text-green-600 mr-1 flex-shrink-0"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <svg
                    className="w-3 h-3 md:w-4 md:h-4 text-red-600 mr-1 flex-shrink-0"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
